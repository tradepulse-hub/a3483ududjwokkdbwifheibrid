...
export default function WorldcoinPriceTester() {
  const { t } = useLanguage()
  const [selectedToken, setSelectedToken] = useState<string>("WLD")
  ...
  const fetchTokenPrice = async (symbol: string) => {
    ...
    try {
      const response = await fetch(`/api/worldcoin-token-price?symbol=${symbol}`)
  ...
  return (
    <div className="bg-gray-900 text-gray-200 rounded-lg p-4 shadow-lg border border-gray-700">
      <h3 className="text-lg font-medium mb-4 text-white">Worldcoin Price API Tester</h3>
