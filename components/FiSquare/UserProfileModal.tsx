  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm overflow-y-auto">
      ...
      <motion.div
        ...
        className="bg-gradient-to-b from-gray-900/95 to-gray-950/95 rounded-xl border border-gray-700/50 shadow-2xl w-full max-w-md mx-4 my-8 relative z-10 max-h-[90vh] overflow-y-auto"
      ...
      >
        ...
        <div className="absolute top-3 right-3 z-20">
          <button
            onClick={onClose}
            className="bg-gray-800/80 hover:bg-gray-700/80 text-gray-300 p-1.5 rounded-full transition-colors"
          >
            <svg
        ...
        <div className="relative">
          <div className="h-32 bg-gradient-to-r from-blue-600/30 via-purple-600/30 to-blue-600/30 relative overflow-hidden">
          ...
          <div className="absolute left-4 -bottom-12">
            <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-800 border-4 border-gray-900 shadow-xl">
              <Image
        ...
        <div className="pt-14 px-4 pb-4">
          ...
          {/* Estatísticas */}
          <div className="flex justify-around mt-4 bg-gray-800/50 rounded-lg p-3 border border-gray-700/30">
            <div className="text-center">
              <div className="text-lg font-bold text-white">{userProfile.postCount}</div>
              <div className="text-xs text-gray-400">{t("posts", "Posts")}</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-white">{userProfile.followers.length}</div>
              <div className="text-xs text-gray-400">{t("followers", "Followers")}</div>
            </div>
            <div className="text-center">
              <div className="text-lg font-bold text-white">{userProfile.following.length}</div>
              <div className="text-xs text-gray-400">{t("following", "Following")}</div>
            </div>
          </div>

          {/* Endereço da carteira */}
          <div className="mt-4 bg-gray-800/50 rounded-lg p-3 border border-gray-700/30">
            <p className="text-xs text-gray-400 mb-1">{t("wallet_address", "Wallet Address")}</p>
            <p className="text-xs font-mono text-gray-300 break-all">{userProfile.address}</p>
          </div>
        </div>
        ...
        <div className="px-4 pb-4">
          <h3 className="text-lg font-semibold text-white mb-3">{t("user_posts", "Posts")}</h3>
          ...
          {userPosts.length > 0 ? (
            ...
            <div className="bg-gray-800/50 rounded-lg p-4 text-center border border-gray-700/30">
              <p className="text-gray-400 text-sm">{t("no_posts_yet", "No posts yet")}</p>
            </div>
          )}
        </div>
      </motion.div>
